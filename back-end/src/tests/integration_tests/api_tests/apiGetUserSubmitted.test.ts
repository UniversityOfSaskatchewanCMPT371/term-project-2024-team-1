...
describe("API Test for /api/survey/:surveyId/user", () => {
  ...
  it("should only retrieve the users who have NOT completed the survey", async () => {
    const response: any = await request(app).get("/api/survey/1/user?type=NOT_COMPLETED").send({ approved: true }).set("Authorization", `Bearer ${USER_TOKEN}`);
    
    expect(response.status).toBe(200);
    expect(response.body).toEqual([{ userId: "4" }, { userId: "5" }, { userId: "test12345" }]);

  });

  it("should indicate that a survey does not exist for when the API is called on a nonexistent survey", async () => {
    const response: any = await request(app).get("/api/survey/100/user?type=COMPLETED").send({ approved: true }).set("Authorization", `Bearer ${USER_TOKEN}`);
