import { sqlPool } from "@app/adapter/SQLRepositories/SQLConfiguration"; // Removed unused 'query'
...
describe("MySQL Database Tests", () => {
 ...
 test("Check number of columns in SurveyQuestions table", async () => {
   ...
   test("Check number of columns in Requests table", async () => {
    const tableName = "Request";
    const expectedColumns = 8; // Adjust as needed
    const [fields] = await sqlPool.query(`DESCRIBE ${tableName}`);
  ...
  test("Check number of columns in SurveyAnswers table", async () => {
    ...
    const expectedColumns = 4; // Adjust as needed
    const [fields] = await sqlPool.query(`DESCRIBE ${tableName}`);
  ...
  test("Check column names in User table", async () => {
    const tableName: string = "User";
    const [fields] = await sqlPool.query(`DESCRIBE ${tableName}`);
    const columnNames: string[] = (fields as RowDataPacket[]).map((field: any) => field.Field);
    const expectedColumnNames: string[] = ["id", "userId", "password", "email", "isAdmin", "clinicName"];
    expect(columnNames).toEqual(expect.arrayContaining(expectedColumnNames));
  });

  test("Check column names in UserAnswerMap table", async () => {
    const tableName = "UserAnswerMap";
  
    const expectedColumnNames = ["id", "user_id","answer_id"];
    const [fields] = await sqlPool.query(`DESCRIBE ${tableName}`);
    const columnNames = (fields as RowDataPacket[]).map(field => field.Field);

    expect(columnNames).toEqual(expect.arrayContaining(expectedColumnNames));
    logger.info(`Column names in ${tableName} table are as expected.`);
  });
    
  test("Check column names in Request table", async () => {
    const tableName = "Request";
  
    const expectedColumnNames = ["id", "clinicName", "email", "password","createdDate", "decisionDate", "requestType"];
    const [fields] = await sqlPool.query(`DESCRIBE ${tableName}`);
    const columnNames = (fields as RowDataPacket[]).map(field => field.Field);
  ...
  test("Check column names in SurveyQuestions table", async () => {
    const tableName = "SurveyQuestions";
  
    const expectedColumnNames = ["id", "question"];
    const [fields] = await sqlPool.query(`DESCRIBE ${tableName}`);
    const columnNames = (fields as RowDataPacket[]).map(field => field.Field);
  ...
  test("Check column names in SurveyAnswers table", async () => {
    const tableName = "SurveyAnswers";
    const expectedColumnNames = ["id", "question_id", "note"]; 
    const [fields] = await sqlPool.query(`DESCRIBE ${tableName}`);
    const columnNames = (fields as RowDataPacket[]).map(field => field.Field);
